...
from .ljspeech_decoder_all import LJSpeechDecoder
...
from ..ttr.summarizer_bs1 import SummarizerTransformer
...
def get_dataset_loaders(h):
    ...
    trainset = CachedEverythingBS1Dataset(
        ...
        return_text=True,
        split_textgrid_folder=h.textgrid_train_root,
        mode='train'
    )
    validset = CachedEverythingBS1Dataset(
        ...
        return_text=True,
        split_textgrid_folder=h.textgrid_val_root,
        mode='valid'
    )
...
def get_models(a, h):
    ...
    # if previous models exist, you shall load previous states to the models.
...
class LJDecoderTTA(pl.LightningModule):
    def __init__(self, models, optims, scheds, h):
        super(LJDecoderTTA, self).__init__()
        ...
        # initialize TTA loss.
        self.tta = SummarizerTransformer.load_from_checkpoint(h.summarizer_path, h=h)

        self.tta.requires_grad_(False)
    ...
    def forward(self, batch):
        the_dict, _ = batch
        return self.generator(code=the_dict['code'], f0=the_dict['f0'])
    
    def training_step(self, batch):
        h = self.h
        optim_g, optim_d = self.optimizers()
        tta_lambda = h.tta_lambda

        the_dict, y = batch
        y_g_hat, hubert_commit_losses, hubert_code_metrics = self.generator(code=the_dict['code'], f0=the_dict['f0'])
        ...
        # input to TTA loss needs to be packed.
        input_to_tta = (the_dict, y_g_hat.squeeze(1))
        loss_tta = self.tta(input_to_tta) * tta_lambda
        ...
        loss_gen_all = loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + loss_mel + loss_tta +hubert_commit_and_codebook_loss
        ...
        self.log("training/gen_tta", loss_tta.item())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        h = self.h
        the_dict, y = batch
        y_g_hat, hubert_commit_losses, hubert_code_metrics = self.generator(**the_dict)
        hubert_commit_and_codebook_loss = hubert_commit_losses[0] * h.commit_lambda
        ...
        loss_mel = F.l1_loss(y_mel, y_g_hat_mel).detach().item() * 45

        input_to_tta = (the_dict, y_g_hat.squeeze(1))
        loss_tta = self.tta(input_to_tta) * h.tta_lambda
        loss_gen_all = loss_mel + loss_tta + hubert_commit_and_codebook_loss

        self.log("validation/gen_mel", float(loss_mel), prog_bar=True, sync_dist=True)
        self.log("validation/gen_tta", float(loss_tta), prog_bar=True, sync_dist=True)
        self.log("validation/gen_commit_codebook", hubert_commit_and_codebook_loss.item(), sync_dist=True)
        self.log("validation/gen_all", float(loss_gen_all), prog_bar=True, sync_dist=True)
...
def get_default_callbacks(a, h):
    ...
    checkpoint_callback_interval = pl.callbacks.ModelCheckpoint(
        ...
        filename="{step}div2",
        save_top_k=-1,
        every_n_train_steps=10000 * 10 # step counting is rather wacky. huh
    ) # and then 
...
if __name__=="__main__":
    ...
    module = LJDecoderTTA(models, optims, scheds, h)
    # ckpt_path = find_most_recent_checkpoint(a, h)
