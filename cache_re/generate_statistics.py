...
def stat_all(dataset_fn, pitch_fn, cache_fn):
    a = h5py.File(dataset_fn,'r')
    audio_files = list(a.keys())
    a.close()
    # audio_files = [os.path.join("local_preprocessed_data/LJSpeech-1.1/wavs_16khz",fn+".wav") for fn in audio_files]

    pitch_wholes = h5py.File(pitch_fn, 'r')
    cache_primary = [(key,pitch_wholes[key][:].flatten()) for key in tqdm.tqdm(audio_files)]
    cache_primary = [(key, e[e!=0]) for key,e in tqdm.tqdm(cache_primary)]
    pitch_wholes.close()
...
if __name__=="__main__":
    ...
    stat_all(
        f"hubert_cache_nonpad_seq/{dataset}/{split}.h5",
        f"f0_cache_whole_seq/{dataset}/{split}.h5",
        f"f0_stats_seq/{dataset}/{split}.npz"
    )