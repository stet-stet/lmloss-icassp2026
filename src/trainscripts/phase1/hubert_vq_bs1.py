...
from ...datasets.cached_hubert_bs1_dataset import HubertCodeBS1Dataset
...
def get_dataset_loaders(h):
    ...
    trainset = HubertCodeBS1Dataset(h.input_training_h5, mode="train")
    validset = HubertCodeBS1Dataset(h.input_validation_h5, mode="valid")
...
def get_models(h):
    return HubertQuantizer(AttrDict(h.code_quantizer))
...
class HubertVQ(pl.LightningModule):
    def __init__(self, models, optims, scheds, h):
        super(HubertVQ, self).__init__()
    ...
    def forward(self, batch):
        return self.generator(code=batch)[0]

    def training_step(self, batch):
        y_g_hat, commit_and_codebook_loss, metrics = self.generator(code=batch)
...
def setup_training():
    ...
    module = HubertVQ(models, optims, scheds, h)
