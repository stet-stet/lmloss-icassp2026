...
from ...loss.asrloss_sampleall import ASRLossSampleall
...
class LJDecoderASR(pl.LightningModule):
    def __init__(self, models, optims, scheds, h):
        super(LJDecoderASR, self).__init__()
        ...
        self.h = h
        self.asr_loss = ASRLossSampleall()
        self.asr_loss.requires_grad_(False)
    ...
    def training_step(self, batch):
        ...
        loss_asr = self.asr_loss(y, y_g_hat) * asr_lambda
        ...
        loss_gen_all = loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + loss_mel + loss_asr +hubert_commit_and_codebook_loss
        ...
        self.log("training/gen_asr", loss_asr.item())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_asr = self.asr_loss(y, y_g_hat) * h.asr_lambda
        loss_all = loss_mel + hubert_commit_and_codebook_loss + loss_asr

        self.log("validation/gen_mel", float(loss_mel), prog_bar=True, sync_dist=True)
        self.log("validation/gen_asr", float(loss_asr), prog_bar=True, sync_dist=True)
...
def get_default_callbacks(a, h):
    ...
    checkpoint_callback_interval = pl.callbacks.ModelCheckpoint(
        ...
        filename="{step}div2",
        save_top_k=-1,
        every_n_train_steps=10000 * 2 # step counting is rather wacky. huh
    ) # and then 
...
if __name__=="__main__":
    ...
    module = LJDecoderASR(models, optims, scheds, h)
    # ckpt_path = find_most_recent_checkpoint(a, h)
