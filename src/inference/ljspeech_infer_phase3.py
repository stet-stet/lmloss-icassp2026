...
from ..trainscripts.phase3.ljspeech_decoder_all import LJSpeechDecoder
...
class Helper(torch.nn.Module):
    ...
    def load_into(self, fn):
        ...
        # keep only the ones with "generator." prefix
...
def cut_to_right_size(f0, secondary):
    # f0 is four times the size of secondary
    # secondary size must be a multiple of 4
...
def setup_inference():
    ...
    for key in tqdm(keys):
        ...
        with torch.no_grad():
            try:
                y, _, _ = generator(f0=f0, code=secondary)
